...
\textbf{Precision}
BFGS attained the smallest distance to the optimum, and both Newton-CG and Trust-Region reached comparable solutions with a slightly larger final distance.
...
\textbf{Evaluation Iterations}  
Trust-Region required the fewest function evaluations, outperforming BFGS and Newton-CG (27 vs 41 vs 107). This suggests that second-order methods generally adapted more efficiently to the problem’s curvature, but Newton-CG struggled due to the function's high condition number of its Hessian matrix.

\textbf{Runtime}  
Trust-Region achieved the fastest runtime, followed by BFGS and Newton-CG.  

\textbf{Precision}  
Trust-Region reached the exact optimum with zero error. BFGS achieved a final distance of $(1.23e-12)$, while Newton-CG had a slightly larger error of $(8.12e-10)$. It indicates that slower convergence may achieve high precision.
